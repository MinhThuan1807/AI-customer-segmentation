 ...
 * UploadSection.tsx
 * -----------------
 * Step 1 of the dashboard: upload a CSV file or use demo data.
 *
 * Features:
 *  - Drag-and-drop upload via react-dropzone
 *  - Accepts .csv files
 *  - Shows file name after selection
 *  - Shows error if file type is invalid
 *  - "Use Demo Data" button to skip upload
 */

import React, { useState, useCallback } from "react";
import { useDropzone } from "react-dropzone";
import { Upload, FileText, AlertCircle, Database, CheckCircle2 } from "lucide-react";
    ...
    <div className="flex flex-col items-center gap-8 w-full max-w-2xl mx-auto">
        ...
        <CardContent className="p-0">
          ...
          >
            ...
            <div className={`
              ...
              transition-colors duration-200
            `}>
              <Upload className={`w-7 h-7 ${isDragActive ? "text-indigo-600" : "text-gray-500"}`} />
            </div>

            {/* Text */}
            <div className="text-center">
              <p className="text-gray-800 mb-1">
                {isDragActive ? "Drop your file here@/app/." : "Drag & drop your CSV file here"}
              </p>
            ...
            <div className="bg-gray-50 rounded-xl px-4 py-2 text-sm text-gray-500 text-center">
              <span className="text-gray-700">Required columns: </span>
      ...
      {fileName && (
        <div className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl border ${
          parsedData ? "bg-emerald-50 border-emerald-200" : "bg-red-50 border-red-200"
        }`}>
          {parsedData ? (
          ...
          <div className="flex-1 min-w-0">
            ...
            {parsedData && (
              <p className="text-xs text-emerald-600">{parsedData.length} customers loaded ✓</p>
            )}
          </div>
        </div>
      )}
      ...
      <div className="flex flex-col sm:flex-row gap-3 w-full">
        ...
        >
          ...
          Use Demo Data (200 customers)
        </Button>
      </div>

      {/* Info about demo data */}
      <p className="text-sm text-gray-400 text-center -mt-2">
        Demo data includes 5 natural customer clusters to help you explore the dashboard instantly.
      </p>
    </div>
  );
}
